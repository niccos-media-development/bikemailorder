...
class StoreHeader extends HTMLElement {
  constructor() {
    ...
    this.desktopSearchToggle = this.querySelector('#desktop-search-alignment-div .js-show-search-left');
  ...
  bindEvents() {
    ...
    if (this.dataset.isSearchMinimised) {
      ...
      if (this.desktopSearchToggle) {
        this.desktopSearchToggle.addEventListener('click', this.handleSearchToggleClick.bind(this));
      }
  ...
  setMenuHeight() {
    if (this.menu && this.menu.clientHeight) {
      let additionalHeight = this.panelButtonsContainer && this.panelButtonsContainer.clientHeight ? this.panelButtonsContainer.clientHeight : 0;

      this.style.setProperty('--menu-height', `${this.menu.clientHeight + additionalHeight + 16}px`);
